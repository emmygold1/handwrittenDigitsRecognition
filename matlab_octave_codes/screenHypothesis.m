...
err_train = zeros(hidden_layer_sizes_length, 1);
err_cv = zeros(hidden_layer_sizes_length, 1);
...
err_train(iter_screen) = mean(double(pred ~= y)) * 100;
pred = predict(Theta1, Theta2, X_cv);
err_cv(iter_screen) = mean(double(pred ~= y_cv)) * 100;
end % end screening hidden_layer_size
figure
hold on
plot(hidden_layer_sizes, err_cv,'.-')
plot(hidden_layer_sizes, err_train, 'r.-')
hold off
save error_screen_hypothesis.mat err_cv err_train
